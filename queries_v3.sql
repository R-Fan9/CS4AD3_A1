...
--*********** 
(select NAME, PRICE, WARRANTYDURATION, TYPE
...
(select NAME, PRICE, WARRANTYDURATION, TYPE
from PRODUCT LEFT OUTJER JOIN WARRANTY)
INTERSECT 
(select NAME, PRICE, WARRANTYDURATION, TYPE
from PRODUCT LEFT OUTJER JOIN WARRANTY
ORDER BY NAME DESC);
...
select * from PRODUCT full OUTER JOIN WITH ON 
with.productID = product.id; 

select * from warranty full OUTER JOIN WITH ON
with.warrantyID = warranty.id;

select * from PRODUCT right OUTER JOIN WITH ON
with.productID = product.id;
...
where p.price >950 AND p.price <1000 and p1.price > 800; 

--2: Find all attendees (their ID, first name, last name) who live in an area with postal code starting with the letter 'N' and have a birth year greater than 1990.  
select a.ID, p.firstName, p.lastName
from Attendee a, Person p, person p1
where a.id = p.id and p.postalCode like 'N%' and year(p1.dateOfBirth) > 1990;
...
--4: Find the total transaction sales (amount) per day of all credit card and debit payments.   List the date and the sales per day.
select sum(t.amount) as totalSales, t.Date
from Transaction t, Transaction t1, Transaction t2
where t.paymentMethod = 'credit' or t1.paymentMethod = 'debit' and t2.paymentMethod = 'credit'
GROUP BY t.Date;
...
--6a: List the vendor name and the number of distinct products that each vendor carries.  For example, Samsung may carry a variety of products across different product categories, such as televisions, laundry machines, smart phones, computers, etc.  Group your results by vendor name.  
select v.name, count(*)
from SELL s, VENDOR v, SELL s1, VENDOR v1
where v.ID = s.vendorID and v1.ID = s1.vendorID
GROUP BY v.name;
...
select v.ID, v.name, SUM(t.quantity) as numProducts, t.Date
from Vendor v, Transaction t, Transaction t1, Transaction t2
where v.ID = t.vendorID AND v.ID = t1.vendorID and v.ID = t2.vendorID
group by v.ID, v.name, t.Date, t1.Date, t2.Date
having SUM(t.quantity) > 10
order by t.Date, sum(t.quantity);
...
where p1.dateOfBirth = p2.dateOfBirth AND
p1.lastName <> p2.lastName AND p1.firstName <> p2.firstName AND p1.lastName <> p3.lastName AND p1.firstName <> p3.firstName AND 
p3.lastName <> p2.lastName AND p3.firstName <> p2.firstName
order by p1.dateOfBirth;
